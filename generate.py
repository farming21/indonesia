...
def build_related_video_card(video):
    cover = cover_for_video(video)
    cover_path = cover if cover.startswith(("http://", "https://")) else "../" + cover
    return f'''<a href="{escape_html(video["slug"])}.html" class="related-card">
    <div class="related-thumb">
        <img src="{escape_html(cover_path)}" alt="{escape_html(video["judul"])}" loading="lazy">
        <span class="related-play" aria-hidden="true">▶</span>
    </div>
    <div class="related-body">
        <h3>{escape_html(video["judul"])}</h3>
        <p>{escape_html(video["tanggal"])}</p>
        <span class="badge">{escape_html(video["kategori"])}</span>
    </div>
</a>'''
...
def generate_video_pages(videos):
    ...
    for v in videos:
        ...
        # Video lainnya: urut terbaru -> terlama berdasarkan tanggal publikasi.
        # Video yang sedang dibuka tidak ditampilkan kembali.
        related_videos = [
